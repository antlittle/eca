...
window.onload = function() {
	getXmlHttp();
	xmlHttp.open("POST", "/eca/user/user_getUser", true);
...
function addUser() {
	...
	xmlHttp.open("POST", "/eca/user/user_addUser", true);
...
function reLoadUser() {
	...
	document.getElementById("userTable").innerHTML = "<tr style='background-color: #68838B; color: white;'><td>账号</td><td>姓名</td><td>电话号码</td><td>注册时间</td><td>操作</td></tr>";
...
function updateUser(event) {
	...
	getXmlHttp();
	xmlHttp.open("POST", "/eca/user/user_updateUser", true);
	var formData = new FormData(updateUserForm);
	formData.append("eca_user_id", event.value);
	xmlHttp.send(formData);
...
function getUserById(event) {
	...
	$("#updateLoadingDiv").removeClass("hideDiv");
	getXmlHttp();
	xmlHttp.open("POST", "/eca/user/user_getUserById", true);
	var formData = new FormData();
	formData.append("eca_user_id", event.value);
	xmlHttp.send(formData);
...
function getUserByIdBack() {
	if (isBack()) {
		...
		$("#updateBtn").val(result.eca_user_id);
		$("#updateLoadingDiv").addClass("hideDiv");
		$("#updateContent").removeClass("hideDiv");
	}
}
...
function deleteUser(event) {
	getXmlHttp();
	xmlHttp.open("POST", "/eca/user/user_deleteUser", true);
	var formData = new FormData();
	formData.append("eca_user_id", event.value);
	xmlHttp.send(formData);
...
function skipToArbitrarilyPage() {
	...
	if (currPage <= 0 || currPage > totalPage) {
		...
		var queryString = document.getElementById("queryString").value;
		getXmlHttp();
		xmlHttp.open("POST", "/eac/user/user_getUser", true);
		var formData = new FormData();
		formData.append("currPage", currPage);
		formData.append("queryString", queryString);
		xmlHttp.send(formData);
